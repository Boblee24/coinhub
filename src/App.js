...
function App() {
  ...
  const handleClick = (articleId, articleOrVideo, clicked) => {
    ...
    const clickedData = articleOrVideo.find(
      (article) => article.article_id === articleId
    ); // Used this to get the elements of the clicked index array
    clicked(clickedData);
  };
  ...
  return (
    <div className=" bg-[#FFFFFF] app relative max">
      <AppContext.Provider
        value={{ scrollToTop, VideosArray, updateData, checkId, handleClick }}
      >
        <Header handleHarmbugerclick={handleHarmbugerclick} toggle={toggle} />
        <Harmbuger
          toggle={toggle}
